# OptimalFit — App Store resubmission kit (UPLOAD 1.6.0 build 39 — see PENDING note)


> **PENDING (2-minute one-time step):** build 39 restores the HealthKit
...
> **Upload the newest archive: `OptimalFit-1.6.0-build39`** — it contains the
...
1. **Upload the build** — Xcode → Window → **Organizer** → Archives → select
   **OptimalFit-1.6.0-build39** (newest in the list) → **Distribute App**
