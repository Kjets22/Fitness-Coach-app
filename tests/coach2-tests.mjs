...
/* ---------------- intake flow (pure engine) ---------------- */
{
  ...
  check("full path ends on free-text preferences", step.id === "anything-else", step.id);
  go("home gym only, hate burpees");
  check("beginner path completes", step === null);
  check("free-text preferences captured", st.freeNotes === "home gym only, hate burpees", st.freeNotes);
  ...
  go2(["squat", "overhead"]); go2(6.5); go2(4); go2("physical"); go2([]); go2(null);
